 ...
 * @Last modified time: 2025-01-30T12:44:09-03:00
 */


/*
For directPathValue and each I could use lodash functions get and each respectively.
...
export function fanOut({obj, directPathChain, addBrokenPaths = false}) {
  ...
  // recursion spread for arrays and objects
    ...
    each(directValue, (o, _i) => {// note near tips, could be formated a collection with some broken paths, and some ok paths
      ...
      const i = _i.toString(); // string or numbers are well handled
...
// nested arrays: [ [0, ['a', 'b']], [1, [0, 1]]]   (last level is a common flat array [index0, index1, ... , indexNm1]instead of array of [[index0, []], ..., [index1, []]]])
export const toCloneNestedIndexes = a => a.map( e => 
  Array.isArray(e)
    ? [e[0], toCloneNestedIndexes(e[1])]
    : e
)
 
//*
export function fanIn({obj, directPathChain, nestedIndexes, flatList, isLevel0 = true}) {
  ...
  const _nestedIndexes = isLevel0 ? toCloneNestedIndexes(nestedIndexes) : nestedIndexes;
  // recursion end cause a leaf
  if ( _directPathChain.length === 0) {
  ...
  // recursion spread for arrays and objects nestedIndexes: [[0, [0, 1, 2, 3]], [1, [0, 1, 2]]]
    ...
    if (ok) {
      ...
      _nestedIndexes.forEach( _indexes => {// elements of: [0, 1] vs [[0, [0,1]], [1, [0,1,2]]]
...
// pack in one more level map, the different already extracted OLeaves, clearing the original plain values arrays v.
export function packOLeaves({obj, directPathChains, addBrokenPaths = false}) {
  const result = {
    l: directPathChains.map( p => extractOLeaves({obj, directPathChain: p, addBrokenPaths}))     
  };

  result.p = extractOLeaves({obj: result.l, directPathChain: '[].v[]', addBrokenPaths})
  result.l = result.l.map( o => ({d: o.d, p: o.p, i: o.i, v: [], b: o.b}));
  //console.log('DBG: ', JSON.stringify(result, null, '\t'));
  return result;
}

export function unpackOleaves({obj, oLeaves, packedOLeaves}) { // modify oLeaves, completes the plain values arrays v
  let ok  = injectOLeaves({obj: oLeaves, processedOLeaves: packedOLeaves});
  let i = 0;
  
  while (ok && i < oLeaves.length) {
    let processedOLeaves = oLeaves[i++];
    //console.log('DBG: ', JSON.stringify(obj, null, '\t'));

    //console.log('DBG: i, ok', i, ok);
    ok &&= injectOLeaves({obj, processedOLeaves});
  }
  
  return ok;
}

export const arrayDistinct = a => [... new Set(a)];
export const distinctTransformedMap = (d, t) => new Map(d.map((k, i) => [k, t[i]]));
export const arrayMapFromTransformedDistinct = (a, d, t) => {
  const dtm = distinctTransformedMap(d, t);
  return a.map( e => dtm.get(e));
}